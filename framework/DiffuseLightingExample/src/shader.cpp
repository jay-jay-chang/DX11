...
bool ShaderDesc_TexDiffuse::Setup(ID3D11Device* device, ShaderBuffer& buffer)
{
...
void ShaderDesc_TexDiffuse::Release()
{
	layout.Release();
	matrixBuffer.Release();
	sampler.Release();
...
bool Shader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	{
		m_desc = new ShaderDesc_TexDiffuse();
...
bool Shader::Render(ID3D11DeviceContext* deviceContext, ShaderParameter& param)
{
	...
	case Shader::DIFFUSE:
		return Render(deviceContext, param, (ShaderDesc_TexDiffuse*)m_desc);
	default:
		break;
	}
	//type error
...
bool Shader::Render(ID3D11DeviceContext* deviceContext, ShaderParameter& param, ShaderDesc_TexDiffuse* desc)
{
	bool result;
	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, param, desc);
...
bool Shader::SetShaderParameters(ID3D11DeviceContext* deviceContext, ShaderParameter& param, ShaderDesc_TexDiffuse* desc)
{
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	BufferObject::MatrixBufferType* dataPtr;
...
void Shader::RenderShader(ID3D11DeviceContext* deviceContext, ShaderParameter& param, ShaderDesc_TexDiffuse* desc)
{
	// Set the vertex input layout.
	deviceContext->IASetInputLayout(desc->layout.m_layout);
